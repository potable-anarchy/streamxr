...
class NeRFManager {
  ...
  async loadNeRF(assetId, modelsDir) {
    ...
    // Build list of paths to check for NeRF files
    const pathsToCheck = [];

    // Look for NeRF files with supported extensions
    for (const ext of this.supportedExtensions) {
      // Check nerf.{ext} in asset directory
      pathsToCheck.push({ path: path.join(assetDir, `nerf${ext}`), ext });
      // Check {assetId}.{ext} in asset directory
      pathsToCheck.push({ path: path.join(assetDir, `${assetId}${ext}`), ext });
      // Check splat/{assetId}.{ext} subdirectory
      pathsToCheck.push({
        path: path.join(assetDir, "splat", `${assetId}${ext}`),
        ext,
      });
      // Check splat/nerf.{ext} subdirectory
      pathsToCheck.push({
        path: path.join(assetDir, "splat", `nerf${ext}`),
        ext,
      });
    }

    console.log(
      `NeRFManager: Checking ${pathsToCheck.length} paths for ${assetId} NeRF...`,
    );

    for (const { path: nerfPath, ext } of pathsToCheck) {
      console.log(`  Checking: ${nerfPath}`);
      if (fs.existsSync(nerfPath)) {
        try {
          ...
          console.log(
            `NeRFManager: ✓ Loaded ${assetId} NeRF from ${nerfPath}: ${buffer.length} bytes`,
          );
          return buffer;
        } catch (error) {
          console.error(
    ...
    console.log(
      `NeRFManager: No NeRF found for ${assetId} (checked ${pathsToCheck.length} paths)`,
    );
    return null;
