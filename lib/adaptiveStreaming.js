...
class AdaptiveStreamingManager {
  constructor() {
    ...
    // Bandwidth simulation state per client: clientId -> { enabled: boolean }
    this.simulationState = new Map();
  }

  /**
   * Enable or disable bandwidth simulation for a client
   * @param {string} clientId - Client identifier
   * @param {boolean} enabled - Whether to enable slow network simulation
   */
  setSimulationMode(clientId, enabled) {
    this.simulationState.set(clientId, { enabled });
    console.log(`Bandwidth simulation ${enabled ? 'ENABLED' : 'DISABLED'} for client ${clientId}`);
  }

  /**
   * Check if bandwidth simulation is enabled for a client
   * @param {string} clientId - Client identifier
   * @returns {boolean} Whether simulation is enabled
   */
  isSimulationEnabled(clientId) {
    const state = this.simulationState.get(clientId);
  ...
  selectLOD(clientId, baseAssetId) {
    // Force low LOD when bandwidth simulation is enabled
    if (this.isSimulationEnabled(clientId)) {
      console.log(`Client ${clientId}: Simulation mode ACTIVE, forcing low LOD`);
  ...
  removeClient(clientId) {
    ...
    this.simulationState.delete(clientId);
